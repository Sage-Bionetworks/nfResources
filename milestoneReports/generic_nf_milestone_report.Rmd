...
###NTAP project view
### syn10147576
#test_name <- unique(deliverables_survery$`Project Name`)[4] #!!!!!! Change the index to grab different projects!
...
query <- paste0("select * from ",  fvid, " where projectId = '",projectid,"' and reportMilestone = '", milestone, "'")
...
if(nrow(test_name_deliverables)==0){
  test_name_deliverables=data.frame(
      ...
      `Assay`=unique(six_mo_fileview$assay),
      `Proposed Number of Samples`=0)
}
