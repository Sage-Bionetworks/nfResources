...
rule all:
  input: 
    ...
    expand(BASE_DIR + "05_disambiguate_to_fastq/{ind}.disambiguatedHuman.fastq", ind = INDIVIDUALS)
...
rule merge_hg38_star_output:
    input:
      ...
      bamlist_hg38 = bamlist_hg38
    output:
      BASE_DIR + "03_merge/output/hg38/{ind}_Aligned.sortedByCoord.out.bam"
    shell:
      "samtools merge {output} {input.bamlist_hg38} "
...
# Function for retrieving list of files with same ind
def bamlist_mm10(individual):
    paths = []
    ind = str(individual)
    for root, dirs, files in os.walk(r"/home/xdoan/MPNST/02_run_STAR/output/mm10", topdown=True):
        for name in files:
            if name.startswith(ind) & name.endswith(".bam"):
                path = os.path.join(root, name)
                paths.append(path)
...
rule merge_mm10_star_output:
    input:
      # i = BASE_DIR + "02_run_STAR/output/hg38/{smp}_Aligned.sortedByCoord.out.bam",
      bamlist_mm10 = bamlist_mm10
    output:
      BASE_DIR + "03_merge/output/mm10/{ind}_Aligned.sortedByCoord.out.bam"
    shell:
      "samtools merge {output} {input.bamlist_mm10} "
...
rule disambig_to_fasta:
    input:
        BASE_DIR + "04_disambiguate/{ind}.disambiguatedSpeciesA.bam"
    output:
        BASE_DIR + "05_disambiguate_to_fastq/{ind}.disambiguatedHuman.fastq"
    shell:
