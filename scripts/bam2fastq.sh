...
for i in `synapse query "select id from syn20629788 where \"parentId\"='syn20734474'"|cut -f 4` ;
	...
	for j in *.bam;
		...
		pigz ${j%.bam}_1.fastq;
		pigz ${j%.bam}_2.fastq;
		synapse store ${j%.bam}_1.fastq.gz -parentid syn20746308 --annotations "$(synapse get-annotations --id $i)";
		synapse store ${j%.bam}_2.fastq.gz -parentid syn20746308 --annotations "$(synapse get-annotations --id $i)";
		rm $j;
